...
class Cifar10DataModule(pl.LightningModule):
    ...
    def setup(self, stage=None):
        # print(self.img_size)
        if stage == 'fit' or stage is None:
            self.train_dataset = CIFAR10(root=self.dataset_dir, train=True, download=True, transform=self.transform)
            self.val_dataset = CIFAR10(root=self.dataset_dir, train=False, download=True, transform=self.transform)
        if stage == 'test' or stage is None:
            self.test_dataset = CIFAR10(root=self.dataset_dir, train=False, download=True, transform=self.transform)
