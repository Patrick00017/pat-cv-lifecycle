...
class TransformerDecoder(nn.Module):
    def __init__(self, n_patches, patch_size, embed_dims, decoder_embed_dims, n_heads, chw, mlp_ratio=4, depth=4):
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, n_patches ** 2, decoder_embed_dims), requires_grad=True)

        self.blocks = nn.ModuleList([TransformerBlock(hidden_dims=decoder_embed_dims, n_heads=n_heads) for _ in range(depth)])
        self.mlp = nn.Sequential(
...
class ViT_Autoencoder(nn.Module):
    def __init__(self, chw=(1, 28, 28), n_patches=7, hidden_dims=128, n_heads=8, n_blocks=4, decoder_hidden_dims=128):
        ...
        self.decoder = TransformerDecoder(n_patches=n_patches, patch_size=self.patch_size, embed_dims=hidden_dims, decoder_embed_dims=decoder_hidden_dims, n_heads=n_heads, chw=chw)
